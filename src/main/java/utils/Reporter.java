...
public abstract class Reporter {
	...
	public String testcaseName, testDescription, authors, category, dataFileName, dataFileType, excelFileName, excelSheetName;
	...
	public synchronized void startReport() {
		...
		htmlReporter.config().setDocumentTitle("Medmantra");
		htmlReporter.config().setEncoding("utf-8");
		htmlReporter.config().setReportName("Medmantra");
	...
	public void reportStep(String desc, String status, boolean bSnap) throws IOException {
		synchronized (test) {
			...
			if (status.equalsIgnoreCase("pass")) {
				test.get().pass(desc, MediaEntityBuilder
						.createScreenCaptureFromPath(".././Snaps/img" + snapNumber + ".png")
						.build());
			}
		}
...
//			}
	...
	public synchronized void endResult() {
		/*
		 * try { if (getDriver() != null) { getDriver().quit(); } } catch
